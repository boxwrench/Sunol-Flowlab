...
### Removed
- **WP4.4 / hydraulics**: Removed the inaccessible reverse-flow support. The `reverse_flow_allowed` field and the negative-head branch of the `GRAVITY` model are gone from `FlowLink`; negative head (downstream higher than upstream) now yields zero forward flow. The support was already unreachable via config — the link schema sets `additionalProperties: false`, so `reverse_flow_allowed` was rejected at load. Updated the negative-head unit test (removed the reverse-allowed case) and the contracts/presentation/outline docs. The topology remains a directed acyclic graph. `RESTRICTED` and forward `GRAVITY` behavior is unchanged. Reverse flow may return only under the ROADMAP "triggered later" contract.
- **WP4.3 / flow modes**: Removed the unimplemented `COMMANDED` flow mode. It is dropped from the `topology.schema.json` `flow_mode` enum and from the `PlantValidator` allowed set, so a link configured with it is now rejected clearly at configuration load rather than silently treated as `RESTRICTED` at full opening. The dead `COMMANDED` branch and `_commanded_warned` flag were removed from `FlowLink`; the defensive unknown-mode fallback is retained. `RESTRICTED` and `GRAVITY` behavior is unchanged. Active rules/contracts (`SIMULATION_RULES.md`, `PROCESS_UNIT_CONTRACTS.md`, `BUILDING_A_PLANT_SIMULATOR.md`) updated; added a validator test asserting rejection. `COMMANDED` may return only under the ROADMAP "triggered later" contract.
...
- **WP4.0 / determinism**: Fixed iteration order by sorting unit ports lexicographically in `FlowSolver` and `StorageUnit` (F-11 fix).
- **WP4.0 / validation**: Enforced schema constraints and validator rules for new gravity properties, and validated that `design_head_m > 0` is required for gravity-mode links.
- **WP4.0 / tests**: Added unit tests for the orifice law table, clamping, reverse flow, and edge cases. Added integration tests for gravity flow self-regulation/equalization, deterministic replay, boundary proration, and insertion-order independence.
