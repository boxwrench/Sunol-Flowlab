...
8. **Testing on push:** Push each work package to main when it's done. CI runs the test suite and config-schema checks on every push; if CI goes red, fix it before building on top. Update EXPECTED_SCRIPTS when you add/remove a test script.
